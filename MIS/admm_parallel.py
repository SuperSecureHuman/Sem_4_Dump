...
class ADMM:
    ...
    def step_parallel(self):
        Parallel(n_jobs=self.numberOfThreads, backend="loky")(delayed(self.solveIndividual)(i) for i in range(self.N-1))
        ...
        # Combine
        Parallel(n_jobs=self.numberOfThreads, backend="loky")(delayed(self.combineSolution)(i) for i in range(self.N-1))
    ...
    def RidgeObjective(self):
        return 0.5 * norm(self.A.dot(self.X) - self.b)**2 + 0.5 * self.alpha * norm(self.X)**2
